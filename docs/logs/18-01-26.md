...
## SRP Refactoring - Admin Status (18/01/26)

### Problem

The initial fix duplicated org admin check logic in 3 places:
- `/login` - queries `organizationmembers`
- `/session` - queries `organizationmembers`
- `/admin-status` - queries `organizationmembers`

This violates Single Responsibility Principle.

### Solution

Reverted `/login` and `/session` to their original state (no `isOrgAdmin`). Instead, the frontend now calls `/admin-status` after authentication.

### Final Architecture

| Endpoint | Single Responsibility |
|----------|----------------------|
| `/login` | Authenticate user, return token + user |
| `/session` | Return current user info |
| `/admin-status` | Check org admin status |

### Files Modified

**Backend (reverted):**
1. `app_v2/routers/auth.py`
   - `/login` - removed `isOrgAdmin` check
   - `/session` - removed `isOrgAdmin` check

**Frontend:**
1. `src/features/auth/authApi.js`
   - Added `getAdminStatus()` method

2. `src/context/AuthContext.jsx`
   - `checkSession()` - calls `/admin-status` after session check, merges `isOrgAdmin` into user
   - `login()` - calls `/admin-status` after login, merges `isOrgAdmin` into user

### Flow

1. User logs in → `/login` returns user + token
2. Frontend calls `/admin-status` → gets `isOrgAdmin`
3. Merges into user state → Sidebar shows Admin tab if `isOrgAdmin: true`
4. On page refresh → `/session` + `/admin-status` → same flow

---

## Circles System Bug Fixes (18/01/26 - Continued Session)

### Issue 1: CORS Errors on `/my-invitations` and `/availability`

**Problem:** CORS errors when accessing circles endpoints, indicating backend crash.

**Root Cause:** `decline_invitation` router was passing `user_id` parameter but service method didn't accept it.

**Fix:**
- `app_v2/routers/circles.py` line 266: Removed extra `user_id` parameter from `decline_invitation()` call

---

### Issue 2: ObjectId Type Mismatch in Queries

**Problem:** Admin panel showed "No pool configured" even though pools existed in database.

**Root Cause:** MongoDB queries were comparing string `user["_id"]` against ObjectId fields in `organizationmembers` collection. Type mismatch caused no results.

**Fix:** Added explicit ObjectId conversion in:
1. `app_v2/routers/auth.py` - `/admin-status` endpoint
2. `app_v2/routers/circles.py` - `/pools` GET endpoint
3. `app_v2/routers/circles.py` - `/invitations/{id}` DELETE endpoint
...
### Issue 3: Missing Create Pool Endpoint

**Problem:** No way to create pools from admin panel - only manage existing ones.

**Fix:** Added:
1. `app_v2/schemas/circles.py` - `CreatePoolRequest` schema
2. `app_v2/routers/circles.py` - `POST /pools` endpoint

---

### Issue 4: Datetime Comparison Error in Accept Invitation

**Problem:** `TypeError: can't compare offset-naive and offset-aware datetimes`

**Root Cause:** `invitation["expiresAt"]` from MongoDB was timezone-naive, but `now = datetime.now(timezone.utc)` was timezone-aware.

**Fix:** `app_v2/services/circles/invitation_service.py` - Added timezone handling:
```python
...
### Issue 5: Datetime Serialization Errors

**Problem:** Various endpoints crashing when serializing datetime fields that might not be datetime objects.

**Fix:** Added robust datetime handling in:
1. `invitation_service.py` - `get_pending_invitations_for_user()`
2. `invitation_service.py` - `get_accepted_invitations_for_user()`
3. `circles.py` - `get_pool_invitations()` endpoint
...
### Issue 6: Hardcoded MIN_GROUP_SIZE

**Problem:** `MIN_GROUP_SIZE = 3` hardcoded in `GroupService`, preventing testing with fewer members.

**Fix:**
1. `app_v2/services/circles/group_service.py` - Read from environment variable:
   ```python
...
### Issue 7: Cancel Invitation Only Worked for Pending

**Problem:** 422 error when trying to delete accepted invitations. Only pending invitations could be cancelled.

**Root Cause:** `cancel_invitation()` had validation requiring `status == "pending"`.

**Fix:** Rewrote `cancel_invitation()` in `invitation_service.py` to:
1. Delete the invitation record entirely (not just change status)
2. Update pool stats based on invitation's status:
   - Decrement `totalInvited`
   - If accepted: also decrement `totalAccepted`
   - If declined: also decrement `totalDeclined`

---

### Issue 8: Pool Stats Not Updating

**Problem:** Pool `stats.totalAccepted` remained 0 after accepting invitation.

**Root Cause:** Invitation belonged to a different pool than the one being viewed in admin panel.

**Resolution:** User education - ensure invitations are sent from the correct pool being managed.

---

### Issue 9: Groups Not Showing for User After Assignment

**Problem:** After admin assigned groups, user's Circles page didn't show their group.

**Root Cause:** Same as above - invitation poolId didn't match pool being managed. User who accepted was correctly added to group, but for a different pool.

**Resolution:** Verified flow is correct when using same pool throughout. User sees group after admin assigns and they refresh.

---

## Debug Logging Added (Temporary)

Added console logging to `CirclesAdmin.jsx` for debugging `canAssign` calculation:
```javascript
console.log('[CirclesAdmin Debug]', {
  acceptedCount,
  minRequired,
  poolStatus: currentPool?.status,
  targetGroupSize: currentPool?.targetGroupSize,
  canAssign,
  invitationsCount: invitations.length,
  invitationStatuses: invitations.map(i => i.status),
});
```

**Note:** Remove this logging before production.
...
| File | Changes |
|------|---------|
| `app_v2/routers/auth.py` | ObjectId conversion in `/admin-status` |
| `app_v2/routers/circles.py` | ObjectId conversions, `POST /pools` endpoint, fixed `decline_invitation`, robust datetime handling |
| `app_v2/schemas/circles.py` | Added `CreatePoolRequest` schema |
| `app_v2/services/circles/invitation_service.py` | Timezone handling in `accept_invitation`, robust datetime serialization, rewrote `cancel_invitation` to delete records |
| `app_v2/services/circles/group_service.py` | `MIN_GROUP_SIZE` and `MAX_GROUP_SIZE` from env vars |
| `.env` | Added `MIN_GROUP_SIZE=1`, `MAX_GROUP_SIZE=6` |

### Frontend (`Deburn-FrontEnd`)

| File | Changes |
|------|---------|
| `src/pages/CirclesAdmin.jsx` | Added debug logging (temporary) |
...
1. **Admin Panel:**
   - View pools for organization
   - Send invitations (with email)
   - View invitation statuses
   - Delete invitations (any status)
   - Assign groups when enough accepted
...
1. **Dashboard `todaysFocus`:** Returns `null` - placeholder for learning module integration
2. **Dashboard `nextCircle`:** Returns `null` - placeholder for next meeting
3. **Email notifications:** Only invitation emails sent, no acceptance/assignment notifications
4. **Meeting reminders:** Code exists but not triggered by cron
5. **Admin meeting management:** Backend supports but frontend doesn't expose

---

## Circles Flow (Verified Working)

```
1. Admin creates pool (via DB or POST /pools)
2. Admin sends invitations from pool
3. Users receive email with accept link
4. Users accept invitation
5. Pool stats.totalAccepted increments
6. Admin clicks "Assign Groups" when enough accepted
7. Groups created in circlegroups collection
8. Users see groups on their Circles page
9. Users can view details, schedule meetings
```
