...
def get_attendees_for_slot(
    ...
    """
    Get user IDs of members available at a specific slot.

    Pure logic function - no DB calls.

    Args:
        member_availability: List from availability doc's memberAvailability
        group_member_ids: List of valid member IDs in the group
        day: Day of week (0-6, Sun-Sat)
        hour: Hour (0-23)

    Returns:
        List of user ID strings who are available at this slot
    """
    attendees = []
    ...
    for member in member_availability:
        ...
        # Check if this member has the requested slot
        slots = member.get("slots", [])
        for slot in slots:
